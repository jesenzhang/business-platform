//! Runs the shared organization behavior contract against the `SQLite`
//! adapter.
//!
...
//! added by `004_runtime_governance_revision1.sql`). Organization rows
//! reference users by bare TEXT ids, so no identity catalog is required.

use identity_authorization_contracts::{verify_organization_contract, OrganizationContractPorts};
use organization_sqlite::{run_migrations, SqliteOrganizationStore};
...
async fn setup() -> sqlx::SqlitePool {
    ...
    run_migrations(&pool)
        .await
        .unwrap_or_else(|_| unreachable!("organization catalog must apply"));
    pool
}

#[tokio::test]
async fn sqlite_organization_contract_suite_passes() {
    let pool = setup().await;
    let store = Arc::new(SqliteOrganizationStore::new(pool));
    ...
    assert!(
        result.is_ok(),
        "organization `SQLite` contract failed: {:?}",
        result.as_ref().err()
    );
}
