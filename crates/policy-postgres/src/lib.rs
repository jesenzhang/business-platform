//! `PostgreSQL` adapter for the Policy bounded context (production
//! authority). Implements the policy command/query ports as audit-carrying
...
/// `PostgreSQL` policy store. One adapter serves both policy ports.
#[derive(Clone)]
...
fn actor_of(audit: &MutationContext) -> Result<AuditActor, PolicyStoreError> {
    let actor_id = Uuid::parse_str(&audit.actor_id).map_err(|_| PolicyStoreError::Failed)?;
...
struct BindingRow {
    ...
    scope_include_subtree: bool,
    scope_resource_kind: Option<String>,
    scope_resource_id: Option<Uuid>,
    status: String,
...
impl BindingRow {
    fn into_binding(self) -> Result<RoleBinding, PolicyStoreError> {
        ...
        let scope = match self.scope_kind.as_str() {
            ...
            "organization_unit" => {
                let Some(org_unit_id) = self.scope_org_unit_id else {
                    return Err(PolicyStoreError::Failed);
                };
                ResourceScope::organization_unit(org_unit_id, self.scope_include_subtree)
            }
            "resource_type" => {
                let Some(kind) = self.scope_resource_kind else {
                    return Err(PolicyStoreError::Failed);
                };
                ResourceScope::resource_type(kind)
            }
            "resource" => {
                let Some(kind) = self.scope_resource_kind else {
                    return Err(PolicyStoreError::Failed);
                };
                let Some(resource_id) = self.scope_resource_id else {
                    return Err(PolicyStoreError::Failed);
                };
                ResourceScope::resource(kind, resource_id)
            }
...
/// The guard is namespaced with `policy:` so the lock keys can never collide
/// with another bounded context's advisory locks.
async fn lock_idempotency(
    ...
    let guard = format!("policy:{operation}|{tenant_id}|{key}");
...
// ---------------------------------------------------------------------------
// command transaction bodies (run inside a caller-owned transaction)
// ---------------------------------------------------------------------------
...
async fn create_role_tx(
    ...
    .await?
    {
        let role = fetch_role_in_tenant(conn, command.tenant_id, existing.result_id).await?;
    ...
    let insert = sqlx::query(
        "INSERT INTO roles (role_id, tenant_id, stable_key, display_name, status, system, created_at, updated_at, version) VALUES ($1,$2,$3,$4,$5,FALSE,$6,$6,1)",
    )
    .bind(role.role_id())
    .bind(command.tenant_id)
    .bind(role.stable_key())
    ...
    // (`PostgreSQL` truncates timestamps to microseconds on storage).
...
async fn set_role_permissions_tx(
    ...
    .await?
    {
        let role_id = existing.result_id;
        let role = fetch_role_in_tenant(conn, command.tenant_id, role_id).await?;
...
async fn bind_role_tx(
    ...
    .await?
    {
        let binding = fetch_binding(conn, command.tenant_id, existing.result_id)
            .await?
            .ok_or(PolicyStoreError::Failed)?;
    ...
    // `IS NOT DISTINCT FROM` compares the nullable scope/window columns
    // `NULL`-safely.
    let scope = scope_columns(&command.scope);
    let existing = sqlx::query_as::<_, BindingRow>(&format!(
        "SELECT {BINDING_COLUMNS} FROM role_bindings WHERE tenant_id = $1 AND user_id = $2 AND role_id = $3 AND scope_kind = $4 AND scope_org_unit_id IS NOT DISTINCT FROM $5 AND scope_include_subtree = $6 AND scope_resource_kind IS NOT DISTINCT FROM $7 AND scope_resource_id IS NOT DISTINCT FROM $8 AND status = 'active' AND effective_at = $9 AND expires_at IS NOT DISTINCT FROM $10",
    ))
    .bind(command.tenant_id)
...
#[async_trait]
impl PolicyCommandPort for PostgresPolicyStore {
    async fn create_role(
        ...
        let outcome = create_role_tx(&mut transaction, &command).await?;
    ...
    async fn update_role(
        ...
        let outcome = update_role_tx(&mut transaction, &command).await?;
        transaction.commit().await.map_err(map_sqlx_error)?;
        Ok(outcome)
    }
    ...
    async fn set_role_permissions(
        ...
        let outcome = set_role_permissions_tx(&mut transaction, &command).await?;
        transaction.commit().await.map_err(map_sqlx_error)?;
        Ok(outcome)
    }
    ...
    async fn bind_role(
        ...
        let outcome = bind_role_tx(&mut transaction, &command).await?;
        transaction.commit().await.map_err(map_sqlx_error)?;
        Ok(outcome)
    }
    ...
    async fn revoke_binding(
        ...
        let outcome = revoke_binding_tx(&mut transaction, &command).await?;
        transaction.commit().await.map_err(map_sqlx_error)?;
        Ok(outcome)
    }
}
...
#[async_trait]
impl PolicyQueryPort for PostgresPolicyStore {
    ...
    async fn list_bindings(
        ...
        let mut query = sqlx::query_as::<_, BindingRow>(&sql).bind(tenant_id);
        if let Some(user_id) = user_filter {
