//! Runs the shared policy behavior contract against the `SQLite` adapter.
//!
...
//! added by `004_runtime_governance_revision1.sql`). Policy's own catalog
//! (schema plus the permission-catalog and immutable system-role seeds) is
//! then applied on top into the same shared in-memory database.

use identity_authorization_contracts::{verify_policy_contract, PolicyContractPorts};
use policy_sqlite::{run_migrations, SqlitePolicyStore};
...
async fn setup() -> sqlx::SqlitePool {
    ...
    run_migrations(&pool)
        .await
        .unwrap_or_else(|_| unreachable!("policy catalog must apply"));
    pool
}

#[tokio::test]
async fn sqlite_policy_contract_suite_passes() {
