...
impl DocumentDetailQuery for SqliteDocumentDetailQuery {
    async fn execute(
        ...
        sqlx::query_as::<_, DocumentRow>(
            "SELECT id, tenant_id, original_filename, content_type, status, version, content_revision, current_revision_id, size_bytes, created_by, created_at, updated_at FROM documents WHERE tenant_id = ?1 AND id = ?2",
        )
