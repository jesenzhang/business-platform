//! The single application composition root (PLAN-0013 Stage 10).
//!
//! `build_app` performs everything the process does before it starts
//! listening: storage client, database adapters for either backend, the
//! platform-authorization and IAM management use-case graphs, the
//! startup-only bootstrap administrator, the OIDC validator, and finally
//! the router. `main` and the end-to-end tests run the exact same code,
//! so a composition that works in tests is the composition that ships.
//!
//! Bootstrap semantics are unchanged (plan §7): HTTP never triggers it,
//! no request surface can enable it, and mode selection is server-side
//! configuration only.
...
use crate::auth::{AuthMiddlewareConfig, ManagementPermission};
use crate::bootstrap::{dev_auth_bootstrap_config, BootstrapComposition};
use crate::config::{BusinessApiConfig, DatabaseBackend, StorageBackend};
use crate::platform_authorization::{
...
/// Build the fully composed HTTP application from validated
/// configuration. Validation is re-checked here (fail fast, no
/// infrastructure touched) so no caller can reach a half-composed
/// router. Returns the router ready to serve.
#[allow(clippy::too_many_lines)]
pub async fn build_app(config: &BusinessApiConfig) -> anyhow::Result<axum::Router> {
    ...
    let state = Arc::new(AppState {
        ...
        processing: Some(crate::state::ProcessingServices {
    ...
    let oidc = if config.auth.issuer_url.trim().is_empty() {
        ...
        Some(std::sync::Arc::new(
            oidc::OidcValidator::with_transport_policy(
    ...
    Ok(routes::create_router(state, auth_config, &config.server))
}
