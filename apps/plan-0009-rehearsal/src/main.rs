...
use plan_0009_rehearsal::{run_inventory, run_stage2, run_stage3, run_stage4, InventoryConfig};
...
async fn run() -> Result<(), &'static str> {
    ...
    let command = arguments
        .next()
        .ok_or("usage: plan-0009-rehearsal inventory|stage2|stage3|stage4")?;
    ...
    let target_root = match command.as_str() {
        ...
        "stage4" => isolation_root.join("stage-4-integrity-recovery-v1"),
        _ => return Err("usage: plan-0009-rehearsal inventory|stage2|stage3|stage4"),
    };
    ...
    if command == "inventory" {
        ...
        let summary = run_stage4(&config).await.map_err(|error| {
            eprintln!("stage4_error={error}");
            error.code()
        })?;
        println!(
            "stage=4 status={} selected={} cases={} fail_closed={} quarantine={} manual_review={} replayed={} manifest_sha256={} mapping_plan_sha256={} matrix_sha256={} formal_facts={} object_files={} duplicate_formal_facts={} duplicate_objects={} audit_file_sha256={}",
            if summary.replayed { "replayed" } else { "frozen" },
            summary.selected_contracts,
            summary.case_count,
            summary.fail_closed,
            ...
            summary.formal_fact_rows,
            summary.object_files,
            summary.duplicate_formal_facts,
            summary.duplicate_objects,
            summary.audit_file_sha256,
        );
    }
    Ok(())
}
