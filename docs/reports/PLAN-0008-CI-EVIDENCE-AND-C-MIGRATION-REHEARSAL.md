...
Final evidence CI run: `31353149398`  
Scope: Final Candidate closeout evidence and read-only migration design. No
...
The preceding evidence-document checkpoint `7966d03587df4908229e3c30737d8e333183fb20`
passed run `31352272475`. The exact final Candidate HEAD
`7eb5421e492a11c0ac20b17f8fd5c3a034f7a29b` passed the final evidence run
`31353149398`:
[GitHub Actions run 31353149398](https://github.com/jesenzhang/business-platform/actions/runs/31353149398).
The final run passed the PostgreSQL + MinIO integration, Architecture Fitness,
...
| --- | --- | --- |
| Stable idempotent upload | Multipart HTTP upload uses the same idempotency key twice; the application contract also checks the same immutable object/revision identity. | PASS in final run `31353149398`. |
| Revision creation and current switch | R1 is created, R2 is created from the aggregate, and current revision changes to R2. | PASS in final run `31353149398`. |
| Historical revision immutability | Direct PostgreSQL update of R1 is rejected by the database integrity fence. | PASS in final run `31353149398`. |
| Exact processing binding | The processing job stores the selected `document_revision_id` and `content_revision`; `ProcessingRun` is started for the same revision. | PASS in final run `31353149398`. |
| Artifact/evidence binding | Artifact points to the run; evidence points to revision, run, and artifact; a cross-revision evidence insert is rejected. | PASS in final run `31353149398`. This is a database identity/FK replay assertion, not a claim that a full review use-case replay was exercised through every application port. |
| Object bytes and metadata/hash | MinIO `HEAD` and `GET` are compared with content length, content type, SHA-256 metadata, and PostgreSQL revision/artifact checksum fields. | PASS in final run `31353149398`. |
| Replay cardinality | Replayed document creation, processing job, run, artifact, and evidence identities remain single rows. | PASS in final run `31353149398` for the exercised persistence identities; artifact/evidence replay uses explicit idempotent SQL conflict handling in the test fixture. |
| Concurrent/stale version rejection | Two writers with one aggregate version race; exactly one save succeeds and the loser receives `RepositoryError::Conflict`. | PASS in final run `31353149398`. |
| Retry convergence | A transient step failure returns the job to `queued`, increments the attempt, and permits a new worker claim. | PASS in final run `31353149398`. |
| Crash recovery and process convergence | Business Worker and AI Worker are killed while leased work is active; fresh processes reclaim the work. Review is replayed and 20 jobs converge to terminal reviewable/succeeded states with one candidate, one review, and one successful AI task for the crash-recovery job. | PASS in final run `31353149398`. |
