...
| 数据治理、分析与可视化 | `DATA_GOVERNANCE_ANALYTICS_AND_VISUALIZATION_ARCHITECTURE.md` | 审计、投影、指标语义、受控查询、Dashboard、报表与 Agent 分析如何统一 |
...
审计、分析投影、指标语义、受控查询和基础可视化属于平台共享能力。业务模块声明领域语义，UI、Open API、报表和 Agent 使用同一指标与权限执行层；Agent 不得直接执行任意 SQL 或成为分析权威。
...
8. 涉及哪些安全和租户边界？
9. 需要满足哪些质量属性场景？
10. 是否产生审计事实、领域事件、分析投影、指标或 Dashboard 声明？
11. UI、Open API、报表和 Agent 是否使用同一受控查询与指标口径？
12. 是否改变公开契约、数据所有权、部署边界或长期技术决策？

缺少以上关键答案时，不应直接以数据库表、统计 SQL、Dashboard 或 Handler 为起点实施。
...
- 是否区分业务状态和执行状态；
- 是否实现必要的幂等、冲突和补偿；
- 是否把指标定义散落在页面 SQL、报表脚本或 Agent Prompt；
- 是否允许 Agent、Dashboard 或导出绕过统一查询、权限和脱敏；
- 是否更新接口、事件、迁移和架构文档；
- 是否增加相应层级的测试；
...
- 引入全局框架或基础设施产品；
- 改变长时任务可靠性语义；
- 改变平台级审计、指标语义、查询或分析存储边界；
- 改变公开 API 或事件的兼容性策略；
- 接受偏离质量属性基线的长期风险。

## 10. 对当前阶段的约束

当前已完成或正在推进的基础计划不负责一次性实现完整数据分析平台，但其结果必须：

- 为上述架构提供可实施的代码骨架；
- 不固化错误的数据所有权；
- 不让基础设施实现进入核心层；
- 不新增任意 SQL Agent Tool、跨上下文共享写模型或不可重建分析权威；
- 通过后续独立 PLAN 分阶段实现审计、投影、语义查询、Dashboard 和 Agent 分析；
- 在合并前同步本架构文档集。

## 11. 合并与采用

本清单及其专题 Baseline 以合并到 `main` 的版本为权威。未合并分支上的文档只能作为候选，不能覆盖当前已接受 Baseline。

实施分支必须在最终审查前同步最新 `main`，解决冲突并重新运行架构门禁。不得以“计划启动时尚无此文档”为理由保留与当前 Baseline 冲突的实现。
